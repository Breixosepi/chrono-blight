...
class FallingTrap:
    def __init__(
        ...
        ts_name = "Tile_green.png" if phase == "green" else "Tile_red.png"
        ts_path = settings.BASE_DIR / "assets" / "graphics" / "tilesets" / ts_name
        
        try:
            tileset_img = pygame.image.load(str(ts_path)).convert_alpha()
            src_x = tile_col * 16
            src_y = tile_row * 16
            self.image.blit(tileset_img, (0, 0), (src_x, src_y, self.width, self.height))
        except Exception:
    ...
    def render(self, surface: pygame.Surface, camera_x: float, camera_y: float) -> None:
        if self.state in ("hidden", "broken") or self.room.player.phase_color != self.phase:
            return
