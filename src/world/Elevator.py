...
class Elevator:
    def __init__(
        ...
        self.tex_open = settings.TEXTURES["elevator_open"]
        self.tex_closed = settings.TEXTURES["elevator_closed"]
        self.image = self.tex_closed
        
        # The elevator will drop from the ceiling, so if state is hidden/descending, we start higher up
        if self.state == "descending":
            self.y = -self.height
            self.hitbox.y = int(self.y)
        elif self.state == "arriving":
            # Start just above the screen/floor so it drops in view
            self.y = self.start_y - self.height - 300
            self.hitbox.y = int(self.y)
            # Instantly snap the player here so the camera starts in the right place
    ...
    def _center_player(self, player: Any) -> None:
        """Helper to center player inside elevator boundaries."""
    ...
    def activate(self) -> None:
        """Called when boss is defeated (or for debugging)"""
    ...
    def update(self, dt: float) -> None:
        ...
        if self.state == "descending":
            ...
            if self.y + self.height < self.room.camera.offset[1]:
            ...
            if self.y + self.height < self.room.camera.offset[1] and not self.room.play_state.in_transition:
    ...
    def render(self, surface: pygame.Surface, camera_x: float, camera_y: float) -> None:
        ...
        # Draw tiled rope from elevator top to screen top
